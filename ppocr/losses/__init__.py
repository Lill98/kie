...
def build_loss(config, weight_class=None):
    support_dict = [
    ...
    if weight_class is not None and "VQASerTokenLayoutLMLoss" == module_name:
        print("build with weight ......", weight_class)
        module_class = eval(module_name)(**config, weight_class=weight_class)
        return module_class
    else:
        module_class = eval(module_name)(**config)
