...
class SimpleDataSet(Dataset):
    def __init__(self, config, mode, logger, seed=None):
        ...
        self.filter_non_linking(self.data_lines)
        
        self.logger.info(f"number of {mode}: " +  str(len(self.data_lines)))  
        ...
        # self.calculate_weight_class()
        # self.weight_class = class_weight.compute_class_weight(class_weight = 'balanced',classes =np.unique(self.list_label),y =np.array(self.list_label))
        self.weight_class =  [ 0.19563016, 5.97586432, 1.9051158, 4.88422146, 1.09971127, 46.91617251, 16.47817855]
        # print("self.weight_class ", self.weight_class )
    ...
    def calculate_weight_class(self):
        ...
        for data_line in self.data_lines:
            try:
                ...
                # print("data['ext_data']", data['ext_data'])
                # print("---------data", data.keys())
                outs = transform(data, self.ops)
                # remove -100 value
                batch_label = np.delete(outs[5], np.where(outs[5] == -100))
                self.list_label.extend(batch_label.tolist())
                # print("outs", outs)
            except:
            #     with open("re_error.txt","a") as f:
            #         f.write(file_name+"\n")
                continue
            
            
    def filter_non_linking(self, data_lines):
        final_data_line = []
        for data_line in data_lines:
            try:
                TAKE = False
                data_line_ = data_line.decode('utf-8')
                substr = data_line_.strip("\n").split(self.delimiter)
                file_name = substr[0]
                file_name = self._try_parse_filename_list(file_name)
                label = substr[1]
                info_dicts = json.loads(label)
                for info_dict in info_dicts:
                    if info_dict["linking"]:
                        TAKE = True
                        break
                if TAKE:
                    final_data_line.append(data_line)
                # print("info_dict", info_dict)
            except:
                continue
        # print("---------len(final_data_line)", len(final_data_line))
        self.data_lines = final_data_line
    ...
    def get_ext_data(self):
        ...
        # print("load_data_ops", load_data_ops)
        ext_data = []
